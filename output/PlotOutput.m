...
%% x_r vs x_m
x_mC = load('Node_Dsp.out');
x_r = load('Node_Dsp_Ref.out');
data = load('Elmt_Frc.out');
f_mC = data(:,4);
data = load('Elmt_Frc_Ref.out');
f_r = data(:,4);
...
V = sum(load('Vbase.out').');
d = x_mC(:,3);
V_R = sum(load('Vbase_Ref.out').');
d_R = x_r(:,3);
